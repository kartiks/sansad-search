...
**Snippet / crop configuration (F05 / F02 — PRD v3.3):** The returned `snippet` is produced at search time in Python by `api/services/search.py` via `_build_snippet`, a sliding word-window over the hit's `full_text_en`, sized to the *effective snippet size* computed per request (see "Effective snippet size" below). **This Python window is the operative snippet lever.** `search.py` still sends Meilisearch query-time parameters (below, not index settings) — the highlight tags drive `<mark>` matching — but the Meilisearch `cropLength` does **not** size the returned `snippet`; it mirrors the effective snippet size for consistency and is non-operative for snippet length:
```json
{
...
```
- The effective snippet size is measured in **words**; `_build_snippet` returns a window of that many words centred on the densest match passage within `full_text_en`. The Meilisearch `cropLength` (also words) is set to the same value but is non-operative for the returned `snippet`.
- When `full_text_en` contains fewer than the effective snippet size in words, `_build_snippet` returns the full text uncropped (no padding) — satisfying the F05 "no truncation, no padding of content shorter than the effective snippet size" rule.
- A match near the start/end of `full_text_en` yields a shorter window (still anchored on the match) — acceptable per F05.
