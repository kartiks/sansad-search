"""
rsdebate.nic.in DSpace provider — RS fallback for items absent from the
sansad.in/rs HTML front end and the Internet Archive mirror.

Discovery:
  DSpace browse ?type=dateissued for rsdebate.nic.in →
...
Bitstream URLs are ALWAYS resolved from the item page — never constructed from a
filename pattern (ARCHITECTURE.md Key Design Patterns).
...
RSDEBATE_BROWSE_URL = f"{RSDEBATE_BASE}/browse"
...
def _resolve_bitstream_url(html: str, base_url: str) -> str | None:
    ...
    Reads the link from the page — never constructs filenames (the rsdebate
    bitstream naming convention is inconsistent across years).
    """
...
class RsdebateDspaceProvider(Provider):
    """
    rsdebate.nic.in DSpace provider — RS fallback.

    Bitstream PDF URLs are resolved from the DSpace item page on demand during
    fetch(); they are never constructed from filename patterns.
    """
    ...
    async def discover(self) -> list[DocumentRef]:
        """
        Paginate the DSpace browse-by-date endpoint and return one DocumentRef
        per item page.

        canonical_doc_id = handle number N (cross-provider join key shared with
        InternetArchiveProvider — if IA already processed N, the checkpoint store
        skips this DocumentRef in the orchestrator).
        """
        ...
        for item_url in item_urls:
            ...
            if handle_n is None:
                logger.warning(
                    "rsdebate_dspace: no handle number in %s; skipping", item_url
                )
                continue
        ...
        logger.info("rsdebate_dspace: discovered %d RS item pages", len(doc_refs))
        return doc_refs
    ...
    async def fetch(self, doc_ref: DocumentRef) -> bytes | None:
        ...
        if resp is None:
            logger.warning("rsdebate_dspace: item page fetch failed for %s", item_url)
            return None
        ...
        if bitstream_url is None:
            logger.warning(
                "rsdebate_dspace: no PDF bitstream link on item page %s; skipping",
                item_url,
            )
            return None
        ...
        if pdf_resp is None:
            logger.warning(
                "rsdebate_dspace: PDF fetch failed for %s (item %s)",
                bitstream_url,
                item_url,
            )
            return None
