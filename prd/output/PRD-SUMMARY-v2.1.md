...
|---|---------|---------------------|--------------|-----------|
| F01 | Data Ingestion | Fetches, parses, segments, and indexes CA/LS/RS records via a two-stage pipeline (--stage fetch/process/all); resumable; --date-from/--date-to scope both stages | None | Rate limiting, storage, processing time, resumability |
...
| F01 → F05 | F05 renders fields indexed by F01; `lang_original`, `time_of_day`, `word_count`, `id`, `sequence_within_sitting` (Q+A) are F01 fields consumed by F05 and F09 |
