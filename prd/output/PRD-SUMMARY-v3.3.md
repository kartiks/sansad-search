...
| F02 | Full-text Search | Keyword search with BM25 ranking, query expansion, and an optional `snippet_size` parameter across all indexed records | F01, F04 | Response time (PERF-1), scalability, snippet size bound (PERF-4) |
...
| F05 | Result Display | Result cards with configurable-size snippets (default 100 words), metadata, language badges, and source links; 20-per-page pagination | F01, F02 | None beyond PERF-1 / PERF-4 |
...
| F02 → F05 | F05 renders ranked results and snippets derived from F02 search response; snippet length is the effective `snippet_size` (F02 param else default 100) |
...
| PERF-4 | `snippet_size` bounded to 20–1000 words (clamped), default 100 operator-configurable; PERF-1 must hold at snippet_size=1000 |
